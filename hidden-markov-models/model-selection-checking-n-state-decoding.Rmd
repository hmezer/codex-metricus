...
header-includes:
  - \usepackage{soul}
  - \usepackage{xcolor}
  - \definecolor{normtext}{HTML}{2D2F92}
---
...
\textcolor{normtext}{\textbf{We consider a time series giving the step lengths of the female elephant Habiba in the package `moveHMM`.}}
...
\textcolor{normtext}{\textbf{To determine the number of states and candidate parameters for the initial distribution, we consult the histogram:}}
...
\textcolor{normtext}{\textbf{We fit a 2-state model:}}
...
\textcolor{normtext}{\textbf{We fit a 3-state model:}}
...
\textcolor{normtext}{\textbf{Finally, we fit a 4-state HMM:}}
...
\textcolor{normtext}{\textbf{Which model is the best fit among these three? We can utilize the information criteria to guide us:}}
...
\textcolor{normtext}{\textbf{When we observe the states, especially in the 4-state setting, it is apparent that the states rather "mop up" the structure.}}

\textcolor{normtext}{\textbf{Now, considering this set of candidate models for the data set we have, how could we select the best one? What criteria to utilize besides the information criteria? Let's plot the marginal distribution of the states to see how well they fir to empirical distributions:}}
...
\textcolor{normtext}{\textbf{Now we move on to the residual checking for the model fit:}}
...
\textcolor{normtext}{\textbf{Now, after checking out these things, and that we are content with our choice of the model with 3-states. We move on to state decoding.}}

\textcolor{normtext}{\textbf{But there is a catch with it! There are two kinds of state decoding, namely, global and local. In local state decoding, we are asking our algorithm to point out what is the most likely state at each individual point t. And for global state decoding, we are asking the respective algorithm of ours which path is the most likely one that could occur. So instead of checking our ass at each step, we are gazing into the horizon and trying to point out which road to follow.}}

\textcolor{normtext}{\textbf{They can be both good and bad, depending on what we expect from them. Local decoding could possible miss the temporal structure and as if 'overfit' to data. But it is quite precise in decoding what is going on at each point, regardless of the neighboring activity. Zig-zagging due to that is inevitable.}}

\textcolor{normtext}{\textbf{Meanwhile, global decoding rather smooths out the state transition process, possibly ignoring local anomalies, so as to be blind to these.}}
...
\textcolor{normtext}{\textbf{We observe here that the probabilities are mostly gradually rising and falling, making it indeed hard to determine the most probable sequence of states.}}

\textcolor{normtext}{\textbf{I wonder, what difference would we observe for the first 50 values if we check the 4-state model. Let's PLOT!}}
...
\textcolor{normtext}{\textbf{From the visual examination, the most we can claim is that 4-state setting makes the state probabilities act more wiggly and less apparent. At least, it can be claimed that the state probabilities seem unsure and rather noisy.}}
